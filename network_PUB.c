...
int AgentGetRatio(NET *net, AGENT *agent, double *ratio)
{
   int idx, nei;
   for (idx = 0; idx < agent->num_agent; ++idx)
   {
      ratio[idx] = agent->strategy[idx];
      for (nei = 0; nei < net->num_neighbour[idx]; ++nei)
      {
	 ratio[idx] += agent->strategy[ net->id_neighbour[idx][nei] ];
      }
      ratio[idx] = ratio[idx] / (net->num_neighbour[idx]+1);
   }
   return 0;
}
...
//   double sigmoid = 1./2.;
   double rn = ((double)(*random_vector+0.01)) / ((double)(RAND_MAX + 0.1));
      ...
      int *strategy_neighbour, double **payoff_neighbour)
{
   ...
   /* 另外, 需要注意这里博弈者如果选择合作, 
    * 那么它所出的成本是上一次它与邻居策略的平均值(其实就是选择1的比例) */
   /* Public */
   int idx;
   double po;
   double sum = 0;
   ...
   {
      if (strategy_neighbour[idx] == 1)
      {
	 sum += RATIO[ id_neighbour[idx] ]; 
      }
   }
   /* 计算每个人得到的payoff */
   ...
   for (idx = 0; idx < num_neighbour; ++idx)
   {
      if (strategy_neighbour[idx] == 1)
      {
	 payoff_neighbour[idx][0] += po - RATIO[ id_neighbour[idx] ];
      }
      else
      {
	 payoff_neighbour[idx][0] += po;
      }
   }
   if (strategy[0] == 1)
   {
      payoff[0] += po - RATIO[id];
   }
   else
   {
      payoff[0] += po;
...
//      AgentPrint(&agent);
      ...
      /* 计算每个agent和周围邻居策略的平均值 */
      AgentGetRatio(&net, &agent, RATIO);
//      for (idx = 0; idx < net.num_node; ++idx)
//      {
//	 printf ( "%d: %f\n", idx, RATIO[idx] );
//      }
	 ...
	 for (idx = 0; idx < agent.num_agent; ++idx)
	 {
	    ...
	    /* 第idx个agent, 它的策略是agent.strategy+idx, 收益是agent.payoff[idx]
	     * 邻居数是net.num_neighbour[idx], 邻居编号是net.id_neighbour[idx]
	     * 邻居策略和收益是 strategy_neighbour, payoff_neighbour */
...
//	 }
	 /* 更新next_strategy */
	 int center;
	 /** 随机选择agent.num_agent个agent进行更新策略, 可能会重复 */
	 for (idx = 0; idx < agent.num_agent; ++idx)
	 {
	    ...
	    /* 第center个agent, 它的策略是agent.strategy+center, 收益是agent.payoff[center]
	     * 邻居数是net.num_neighbour[center]
	     * 邻居策略和收益是 strategy_neighbour, payoff_neighbour 
	     * 用以决定策略的随机数random_vector, 这里需要两个 */
	    StrategyGeneration(center, agent.strategy+center, agent.payoff[center], 
